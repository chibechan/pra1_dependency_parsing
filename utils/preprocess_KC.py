...
def make_df_KC_from_rel(data_path="./data/KyotoCorpus4.0/dat/rel/"):
    '''
    input:
        KCの係り受け解析のデータへのpath
    output:
    ...
    k2ks = []
    k2bs = []
    for file_num in sorted(os.listdir(data_path)): 
            # 開くファイルの名前↓
        data_file = data_path +  '/' + file_num
        print(data_file)
       
        with  open(data_file, encoding = "euc_jp") as f:
            sen = f.readline()
            while(sen):
                sen = sen.strip()
                if (sen == 'EOS'):  # 一文の終わり
...
def make_df_KC_from_syn(data_path="./data/KyotoCorpus4.0/dat/syn/"):
    '''
    input:
        KCの係り受け解析のデータへのpath
    output:
